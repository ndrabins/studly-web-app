
//Authentication Actions
export const SIGN_OUT_USER = 'SIGN_OUT_USER';
export const AUTH_ERROR = 'AUTH_ERROR';
export const AUTH_USER = 'AUTH_USER';
export const AUTH_SUCCESS = 'AUTH_SUCCESS';
export const LOGIN_WITH_PROVIDER_FIREBASE = 'LOGIN_WITH_PROVIDER_FIREBASE';

//Course actions
export const ADD_COURSE = "ADD_COURSE";
export const ADD_COURSE_ERROR =  "ADD_COURSE_ERROR";

export const CREATE_COURSE = "CREATE_COURSE";
export const LEAVE_COURSE = "LEAVE_COURSE";
export const SELECT_COURSE = "SELECT_COURSE";

export const FETCH_ALL_COURSES_REQUEST = "FETCH_ALL_COURSES_REQUEST";
export const FETCH_ALL_COURSES_FAILURE = "FETCH_ALL_COURSES_FAILURE";
export const FETCH_ALL_COURSES_SUCCESS = "FETCH_ALL_COURSES_SUCCESS";

export const FETCH_COURSE_SUCCESS = "FETCH_COURSE_SUCCESS";

//Assignment Actions
export const CREATE_ASSIGNMENT = "ADD_ASSIGNMENT";
export const DELETE_ASSIGNMENT = "DELETE_ASSIGNMENT";

export const FETCH_ASSIGNMENTS_REQUEST = "FETCH_ASSIGNMENTS_REQUEST";
export const FETCH_ASSIGNMENTS_FAILURE = "FETCH_ASSIGNMENTS_FAILURE";
export const FETCH_ASSIGNMENTS_SUCCESS = "FETCH_ASSIGNMENTS_SUCCESS";

export const FETCH_USER_ASSIGNMENTS_REQUEST = 'FETCH_USER_ASSIGNMENTS_REQUEST';
export const FETCH_USER_ASSIGNMENTS_SUCCESS = 'FETCH_USER_ASSIGNMENTS_SUCCESS';

export const FETCH_ASSIGNMENT_DETAIL = "FETCH_ASSIGNMENT_DETAIL";
export const FETCH_ASSIGNMENT_DETAIL_SUCCESS = "FETCH_ASSIGNMENT_DETAIL_SUCCESS";

//Profile Actions
export const UPDATE_PROFILE = "UPDATE_PROFILE";
export const UPDATE_PROFILE_IMAGE = "UPDATE_PROFILE_IMAGE";
export const FETCH_PROFILE = "FETCH_PROFILE";

//Announcement Actions
export const CREATE_ANNOUNCEMENT = "ADD_ANNOUNCEMENT";
export const DELETE_ANNOUNCEMENT = "DELETE_ANNOUNCEMENT";

export const FETCH_ANNOUNCEMENTS_REQUEST = "FETCH_ANNOUNCEMENTS_REQUEST";
export const FETCH_ANNOUNCEMENTS_FAILURE = "FETCH_ANNOUNCEMENTS_FAILURE";
export const FETCH_ANNOUNCEMENTS_SUCCESS = "FETCH_ANNOUNCEMENTS_SUCCESS";

//App utility actions.
export const TOGGLE_SIDENAV = "TOGGLE_SIDENAV"

//CHAT ACTIONS
export const CREATE_COURSE_CHAT = "CREATE_COURSE_CHAT";
export const CREATE_CHANNEL = "CREATE_CHANNEL";

export const FETCH_COURSE_CHANNELS_REQUEST = "FETCH_COURSE_CHANNELS_REQUEST";
export const FETCH_COURSE_CHANNELS_SUCCESS = "FETCH_COURSE_CHANNELS_SUCCESS";

export const FETCH_CHANNEL_MESSAGES_SUCCESS = "FETCH_CHANNEL_MESSAGES_SUCCESS";
export const FETCH_CHANNEL_MESSAGES_REQUEST = "FETCH_CHANNEL_MESSAGES_SUCCESS"

export const SELECT_CHANNEL = 'SELECT_CHANNEL';
export const CREATE_MESSAGE = 'CREATE_MESSAGE';
export const DELETE_MESSAGE = 'DELETE_MESSAGE';
